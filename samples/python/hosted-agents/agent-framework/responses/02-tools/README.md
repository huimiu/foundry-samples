...
An [Agent Framework](https://github.com/microsoft/agent-framework) agent hosted on Microsoft Foundry using the **Responses protocol**. It demonstrates how to register locally defined Python tools with the agent so the model can call them during a conversation.
...
2. The `get_weather` function is decorated with `@tool(approval_mode="never_require")`, accepts a `location`, and returns randomized demo weather
3. The client is wrapped in an Agent Framework `Agent` with the local tool and a short system prompt (*"You are a friendly assistant. Keep your answers brief."*)
...
Define a Python function with the `@tool` decorator, give it type annotations and a docstring, then add it to the `tools` list passed to `Agent`. The model sees the tool name, signature, and descriptions and decides when to call it.
...
The included `get_weather` tool is intentionally simple and returns randomized demo data; replace it with real business logic for production scenarios.
