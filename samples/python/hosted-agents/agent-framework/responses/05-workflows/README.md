...
An [Agent Framework](https://github.com/microsoft/agent-framework) workflow hosted on Microsoft Foundry using the **Responses protocol**. It demonstrates how to compose a linear workflow of specialized agents — a slogan writer, legal reviewer, and formatter — and expose the final workflow as a hosted agent.
...
2. Three specialized Agent Framework `Agent` instances are created: `writer`, `legal_reviewer`, and `formatter`
3. Each agent is wrapped in an `AgentExecutor` with `context_mode="last_agent"`, so each step sees only the previous agent's output
4. `WorkflowBuilder` connects the executors in a linear pipeline (`writer` → `legal_reviewer` → `formatter`) and returns only the formatter's output
5. The workflow is converted to an agent with `.as_agent()` and served by `ResponsesHostServer`, which exposes an OpenAI-compatible `POST /responses` endpoint on `http://localhost:8088/`
...
| `AZURE_AI_MODEL_DEPLOYMENT_NAME` | Yes | Model deployment name. This sample is tested with a more capable model such as `gpt-5.4` and declares a model resource in `agent.manifest.yaml` |
...
- A model capable of continuing a workflow from assistant messages; this sample is tested with OpenAI's `gpt-5.4`
...
This sample requires a more advanced model because the model needs to continue the conversation from an assistant message. Not all models perform well in this scenario; it has been tested with OpenAI's `gpt-5.4`.

The workflow exposes only the final formatted slogan. If you remove `output_executors=[format_executor]`, the response can include intermediate writer and legal-reviewer outputs as well.
