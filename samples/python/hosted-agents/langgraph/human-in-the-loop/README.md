...
**Bash:**
```bash
azd ai agent invoke --local '{"input": "Ask the user where they are, then look up the weather there.", "stream": false}'
```
...
```bash
azd ai agent invoke '{"input": "Ask the user where they are, then look up the weather there.", "stream": false}'
```
