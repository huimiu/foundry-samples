# Text Search RAG Agent (Responses Protocol)

A customer-support assistant built with the [Agent Framework](https://github.com/microsoft/agent-framework) and hosted with the **Responses protocol**. It demonstrates retrieval augmented generation (RAG) by grounding responses in a local text-search knowledge base tool.
...
2. `AIProjectClient.AsAIAgent(...)` creates an outdoor-equipment support assistant backed by the configured Foundry model
3. `AIFunctionFactory.Create(SearchKnowledgeBase, "SearchKnowledgeBase", ...)` registers a local search function as a tool
4. `SearchKnowledgeBase` searches mocked support content for return policy, shipping, tent care, warranty, and hiking boot guidance
...
The sample uses an in-memory dictionary as a mock knowledge base. It includes support content for return policy, shipping, tent care, warranty, and hiking boots. In production, replace `SearchKnowledgeBase` with a vector store, Azure AI Search index, or other retrieval system.

Additional useful prompts include:

```text
How long does shipping take?
How do I clean my tent?
What does the warranty cover on my hiking boots?
```
...
### Answers are not grounded in support content

Ask about topics present in the mock knowledge base, such as return policy, shipping, tent care, warranty, or hiking boots. For other topics, `SearchKnowledgeBase` returns `No relevant information found in the knowledge base.`
