...
An instrumented Agent Framework hosted agent using the **Responses protocol** in C#. It demonstrates Foundry hosted-agent observability with model, agent, and local tool spans.
...
2. `AIProjectClient.AsAIAgent(...)` creates a brief weather assistant backed by the configured Foundry model
3. `AIFunctionFactory.Create` registers two local tools: `GetCurrentLocation` and `GetWeather`
4. Agent Framework for .NET emits OpenTelemetry spans for agent invocation, model chat, and tool execution; `AddFoundryResponses` enables the hosting instrumentation automatically
...
| `AZURE_AI_MODEL_DEPLOYMENT_NAME` | No | Model deployment name. Defaults to `gpt-4.1-mini`. Declared in `agent.yaml` |
| `APPLICATIONINSIGHTS_CONNECTION_STRING` | No | Enables local telemetry when set. Auto-injected in hosted containers |
| `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT` | No | Set to `true` to include prompt and completion content in GenAI trace events. Declared in `agent.yaml` |
...
- Application Insights if you want local telemetry exported outside the hosted environment
...
curl -sS -X POST http://localhost:8088/responses \
  ...
  -d '{"input": "What is the current weather?", "stream": false}' | jq .
```
...
Several spans are created for each request:

- `invoke_agent` captures the full agent invocation
- `chat` captures the model call
- `execute_tool` captures local tool execution for `GetCurrentLocation` and `GetWeather`

For more information, see the [OpenTelemetry GenAI Semantic Conventions](https://opentelemetry.io/docs/specs/semconv/gen-ai/). After deployment, traces, metrics, and logs are collected by Foundry and are available through Azure Monitor, Application Insights, and the **Traces** tab in the Foundry UI.
...
### No telemetry appears locally

For local runs, set `APPLICATIONINSIGHTS_CONNECTION_STRING` before starting the agent. Hosted Foundry containers inject it automatically.
