# Simple Agent (Responses Protocol)

A general-purpose AI assistant — the simplest hosted agent using `AsAIAgent(model, instructions)` via the [Agent Framework](https://github.com/microsoft/agent-framework). Hosted on Microsoft Foundry with the **Responses protocol**, it is the recommended starting point for understanding inline agent creation in C#.

## How It Works

1. `Program.cs` creates an `AIProjectClient` pointed at your Foundry project endpoint, authenticated with `DefaultAzureCredential`
2. `.AsAIAgent(model, instructions, name, description)` turns it into a single general-purpose `AIAgent` with a short system prompt
3. `AgentHost.CreateBuilder(args)` plus `AddFoundryResponses(agent)` and `MapFoundryResponses()` host the agent behind the OpenAI-compatible `POST /responses` endpoint
4. No custom tools or workflows are involved — this is the minimal "just talk to a model" integration
