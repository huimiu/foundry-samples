# Local Tools Agent (Responses Protocol)

A hotel concierge assistant built with the [Agent Framework](https://github.com/microsoft/agent-framework) and hosted with the **Responses protocol**. It demonstrates how C# functions registered with `AIFunctionFactory.Create` can be invoked as local tools by a hosted agent.
...
2. `AIProjectClient.AsAIAgent(...)` creates a Seattle hotel concierge agent backed by the configured Foundry model
3. `AIFunctionFactory.Create(GetAvailableHotels, "GetAvailableHotels", ...)` registers the local C# hotel-search function as a tool
4. When the model chooses the tool, the Agent Framework runs `GetAvailableHotels` in the agent process and feeds the result back to the model
...
Local tools are regular C# methods registered with `AIFunctionFactory.Create`. This sample exposes `GetAvailableHotels`, which returns a mocked list of Seattle hotels with location, nightly price, rating, amenities, and available rooms.

To add another tool, create a static method with simple JSON-serializable parameters and return values, then add another `AIFunctionFactory.Create(...)` entry to the `tools` collection passed to `AsAIAgent`.
...
### The hotel tool is not called

Ask for hotel availability, amenities, locations, ratings, or pricing so the model has a reason to invoke `GetAvailableHotels`. If the response is generic, try a more specific prompt such as `Which Seattle hotels have a pool and spa under $300 per night?`.
