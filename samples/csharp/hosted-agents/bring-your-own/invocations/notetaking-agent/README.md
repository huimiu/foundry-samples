...
A bring-your-own hosted note-taking agent in C# using the **Invocations protocol** and `Azure.AI.AgentServer.Invocations`. It uses Foundry Responses API function calling to save and retrieve session-scoped notes, then streams the final response as SSE.

## How It Works

1. `Program.cs` creates a Foundry `ProjectResponsesClient` and starts `InvocationsServer.Run<NoteTakingHandler>()` on `http://localhost:8088/`.
2. `POST /invocations` reads a JSON request body with a non-empty `message` field.
3. The model can call two local tools: `save_note` writes a note, and `get_notes` retrieves notes for the current `agent_session_id`.
4. [NoteStore.cs](NoteStore.cs) stores notes in `$HOME` as `notes_{session_id}.jsonl`, making them available across requests with the same session ID.
5. The handler streams `token` SSE events as the final answer is generated, then sends a `done` event with invocation and session metadata.
...
Use the same `agent_session_id` to keep reading and writing the same note file:
...
Start a new session by using a different `agent_session_id` or clearing the conversation in Agent Inspector.
